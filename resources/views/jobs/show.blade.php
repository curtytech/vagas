...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="mt-2 text-neutral-600">Empresa: {{ $job->company_name }}</div>
    ...
    <!-- Candidatar-se -->
    @if(auth()->check() && auth()->user()->role === 'employee' && !empty($application))
    <a href="{{ \App\Filament\Resources\JobApplicationResource::getUrl('edit', ['record' => $application->id]) }}"
        class="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-neutral-900 text-white hover:bg-black mt-5">
        Ver candidatura
